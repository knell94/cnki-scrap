...
class paper:
    def __init__(self, name, attribute, references, year):
        ...
        for element in attribute:
            if re.match('摘要',element):
                self.abstract = element
            if re.match('作者\:',element):
                self.author = element
            if re.match('关键词',element):
                self.key = element
            if re.match('类别',element):
                self.cata = element
            if re.match('其他信息',element):
                self.refer = element
...
time.sleep(3)
web = driver.get("http://apps.webofknowledge.com/Search.do?product=UA&SID=U15zderpOWtQ2nkemiu&search_mode=GeneralSearch&prID=ec9f31fa-42d2-4272-8036-3951fffaf86d")
...
for searchpage in range(1,100):
    ...
    for link in thispageurl:
        ...
        for element in elements:
            if element.text.find('出版年') != -1:
                years = element.text.split(':')[1]
                years = years.split()
                for thing in years:
                    if re.match('\d{4}', thing):
                        year = thing
        reference = driver.find_elements_by_css_selector("a[title='查看此记录的题录信息']")
        reflink = reference[0].get_attribute("href")
        #time.sleep(2*random.random())
        #driver.close()
        linkstarttime = int(time.time())
        driver.get(reflink)
        ...
        max = int(driver.find_element_by_id('pageCount.bottom').text)
        listtemp = []
        list = []
        ...
        for i in range(1,max+1):
            ...
            for j in range(len(ref), int(len(ref)/2), -1):
                del(ref[j-1])
            for name in ref:
                tempyear = name.find_element_by_xpath('./../..')
                tempyear = tempyear.find_element_by_xpath('.//span[contains(text(),"出版年:")]')
                tempyear = tempyear.find_element_by_xpath('.//following-sibling::span[1]').text
        ...
        #    element = listtemp[i]
        #    if re.match('查找 Related Records >', element):
        #        deli.append(i)
        #for j in deli[::-1]:
        #    del(listtemp[i])
        #while '' in listtemp:
